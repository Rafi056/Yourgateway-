...
export function Navbar() {
  ...
  const navLinks = [
    { href: "/institutions?type=university", label: "الجامعات", icon: GraduationCap },
    { href: "/institutions?type=language_center", label: "معاهد اللغة", icon: BookOpen },
    { href: "/dashboard", label: "طلباتي", icon: LayoutDashboard },
  ];
            ...
            {navLinks.map((link) => {
              ...
              return (
                <Link 
                  key={link.href} 
                  href={link.href}
                  className={`flex items-center gap-2 text-sm font-medium transition-colors hover:text-primary flex-row-reverse ${
